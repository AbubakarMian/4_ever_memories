...
<div class="table-container">
    <table class="fhgyt" id="testimonialTableAppend" style="opacity: 0">
        <thead>
            <tr>
                <th>Subject</th>
                <th>Title</th>
...
<script>
    $(document).ready(function() {
        ...
        function fetchRecords() {
            $.ajax({
                url: '{!!asset("admin/testimonial/get_testimonial")!!}',
                type: 'get',
                dataType: 'json',
                success: function(response) {
                    console.log('response', response);
                    $("#testimonialTableAppend").css("opacity", 1);
                    ...
                    for (var i = 0; i < len; i++) {
                        ...
                        var edit = `<a class="btn btn-info" href="{!!asset('admin/testimonial/edit/` + id + `')!!}">Edit</a>`;
                        
                        createModal({
                            id: 'testimonial_' + res[i].id,
                            header: '<h4>Delete</h4>',
                        ...
                        var delete_btn = `<a class="btn btn-warning" data-toggle="modal" data-target="#` + 'testimonial_' + res[i].id + `">Delete</a>`;
                        ...
                        $("#testimonialTableAppend tbody").append(tr_str);
                    }
                    
                    // Initialize DataTable after populating data
                    $('#testimonialTableAppend').DataTable({
    ...
    function delete_request(id) {
        $.ajax({
            url: "{!!asset('admin/testimonial/delete')!!}/" + id,
            type: 'POST',
            dataType: 'json',
            data: {
                _token: '{!!@csrf_token()!!}'
            ...
            success: function(response) {
                ...
                if(response){
                    var myTable = $('#testimonialTableAppend').DataTable();
